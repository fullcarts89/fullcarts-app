# FullCarts First Content Batch — 5 Ready-to-Film Scripts

**Date:** June 10, 2026
**Parent:** `docs/plans/2026-06-10-face-forward-content-strategy.md` · rules in `content-rules.md` · workflow in `production-playbook.md`
**Format:** the repeatable hybrid template (hook → proof → context → payoff → CTA). Every script is annotated with the 5 non-negotiables so you can see the rules in action, then reuse the shape forever.

> **⚠️ Accuracy note (read before filming):** All numbers below are pulled live from the FullCarts DB on 2026-06-10. The database currently holds **2,228 documented shrink events across 938 brands and 1,881 products** — *not* the "3,000+" some older docs say. Since accuracy is the entire credibility play, **say "2,200+ documented shrink events" (or "nearly 1,900 products / 900+ brands")** on camera, never an inflated number. One overstated stat that a commenter fact-checks undoes the whole authority position. (Re-pull before each batch; the number grows.)

---

## The repeatable template (copy this for every new video)

```
TITLE:        [working title]
BUCKET:       Educational / Newsjack / Personal / Entertainment / Promo
...
## Script 1 — "The coffee can that didn't change"  (Educational · the authority core)

**Data:** Folgers Coffee 51 oz → 43.5 oz (−14.7%), FullCarts entry, observed 2022-06. Ties to real coffee-commodity inflation (macro). **This is a ~1-in-5 day-job-signature video.**
...
## Script 2 — "Guess how much smaller"  (Entertainment · top-of-funnel reveal)

**Data:** Gatorade 32 fl oz → 28 fl oz (−12.5%), FullCarts entry. (Also true at 20oz → 16.9oz, −15.5% — mention as the kicker.)
...
## Script 3 — "CPI just dropped" (Newsjack TEMPLATE · the reach engine)

**Data:** the monthly **BLS CPI grocery print** (fill in on release day) + your worst-mover category. This is a *fill-in-the-blank* template — keep it ready and fire it within hours of the data drop. **Reserve an X version too** (chart + one-line take).

| Beat | Script |
|---|---|
| **HOOK (0–3s)** | *(face)* "Grocery prices just officially went up **[X]%** this month. But that number is lying to you by leaving something out." **Text overlay:** `CPI groceries +[X]%` |
...
## Script 4 — "Why I built a database of 2,200 shrinks" (Personal · parasocial trust)

**Data:** the database itself — 2,228 events / 938 brands / 1,881 products. The origin story. Run ~monthly; it converts watchers into followers.
...
## Script 5 — "5 things that shrank and hoped you wouldn't notice" (Rundown · Entertainment/Educational)

**Data:** five real FullCarts entries, rapid-fire. Refresh the five each month from the brief generator.
...
6. **Checklist:** 5/5 non-negotiables + evidence-bucket gate on each before it leaves the queue.

After this batch you have the template, the overlay components, and the asset kit — so batch #2 is half the work.
