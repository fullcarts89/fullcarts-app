...
- `docs/content/youtube-strategy.md` — **YouTube layer**: Shorts funnel → ≥1 long-form/month (the searchable evergreen authority piece) → Community-tab carousels/polls. 5 long-form formats + search-title patterns + playlists.
...
- `Carousel` (4:5, 1080×1350) — **still IG swipe carousel**: cover → ranked product slides → CTA. **Photo-forward** when a real photo is present (before/after photo is the hero + mono `before → after` + `−X%`); bars fallback otherwise. Single paired photo (`image`) or explicit `beforeImage`/`afterImage` pair; optional `caught` "first caught" date stamp. Render stills `0..N+1`.
- `CarouselVideo` / `CarouselVideoVertical` — **video** version (auto-advancing → Reel/Short/TikTok). One component → both **4:5** and **9:16** (height-responsive; `calcCarouselVideoMeta`). Folgers-style motion (count-up + badge pop, rank stamp, staggered reveals, before/after slide-in, photo scale-in + Ken Burns), progress bar, `caught` stamp, optional looped `music` bed (`<Audio loop>`, low mix). Renders offline once photos are local.
...
- **Coffee 5-pack built (2026-06-13):** "5 Stealth Coffee Shrinks" — real before/after photos in `video/public/coffee/`, props `coffee-5.json` (video) + `coffee-5-carousel.json` (stills), music `public/audio/bg-loop.mp3`. Dunkin'→Peet's→Community→Eight O'Clock→Folgers (11.3→9.6oz, −15%). Folgers/Dunkin' are the strong anchors; Peet's "before" is a current Walmart 12oz listing (flag if a commenter nitpicks).
