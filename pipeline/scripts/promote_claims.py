...
The /admin/claims Evidence tab separates these from evidence-wall tagged
claims via an `evidence_tags IS NOT NULL` filter rather than a separate
status value.
...
def promote_claims(sb, claims: List[Dict], dry_run: bool = False) -> Dict[str, int]:
    ...
    for i, claim in enumerate(claims):
        try:
            ...
            # to 'evidence'. The /admin/claims Evidence tab filters fold-ins
            # out via an `evidence_tags` predicate so the bucket stays useful
            # for the evidence-wall channels (Skimpflation / So Smol / etc.).
